...
def log_detection(save_file, source_name, conf, x1, y1, x2, y2):
    with open(save_file, "a") as f:
        f.write(f"{source_name},Gun,{conf:.2f},{x1},{y1},{x2},{y2},{time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        ...
        for result in results:
            for box in result.boxes:
                ...
                cv2.putText(annotated_frame, f"Gun: {conf:.2f}", (x1, y1 - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                log_detection("results.txt", "webcam", conf, x1, y1, x2, y2)
...
def main():
    ...
    parser.add_argument("--source", type=str, required=True, 
                        help="Path to image or 'webcam' for live detection")
